...
//------------------------Parameter----------------------
`define TV_IN_src "../tv/cdatafile/c.sobel.autotvin_src.dat"
`define TV_IN_dst "../tv/cdatafile/c.sobel.autotvin_dst.dat"
parameter ADDR_WIDTH = 6;
parameter DATA_WIDTH = 32;
parameter src_DEPTH = 1;
reg [31 : 0] src_OPERATE_DEPTH = 0;
parameter src_c_bitwidth = 64;
parameter dst_DEPTH = 1;
reg [31 : 0] dst_OPERATE_DEPTH = 0;
parameter dst_c_bitwidth = 64;
parameter START_ADDR = 0;
parameter sobel_continue_addr = 0;
parameter sobel_auto_start_addr = 0;
parameter src_data_in_addr = 16;
parameter dst_data_in_addr = 28;
...
reg  BREADY_reg = 0;
reg [src_c_bitwidth - 1 : 0] mem_src [src_DEPTH - 1 : 0];
reg src_write_data_finish;
reg [dst_c_bitwidth - 1 : 0] mem_dst [dst_DEPTH - 1 : 0];
reg dst_write_data_finish;
...
//write src reg
reg [31 : 0] write_src_count = 0;
reg write_src_run_flag = 0;
reg write_one_src_data_done = 0;
//write dst reg
reg [31 : 0] write_dst_count = 0;
reg write_dst_run_flag = 0;
reg write_one_dst_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & src_write_data_finish & dst_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        src_write_data_finish <= 0;
        write_src_run_flag <= 0; 
        write_src_count = 0;
        count_operate_depth_by_bitwidth_and_depth (src_c_bitwidth, src_DEPTH, src_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_src_run_flag <= 1; 
            write_src_count = 0;
        end
        if (write_one_src_data_done === 1) begin
            write_src_count = write_src_count + 1;
            if (write_src_count == src_OPERATE_DEPTH) begin
                write_src_run_flag <= 0; 
                src_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_src
    integer write_src_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] src_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = src_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_src_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write src data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (src_c_bitwidth < 32) begin
                        src_data_tmp_reg = mem_src[write_src_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < src_c_bitwidth) begin
                                src_data_tmp_reg[j] = mem_src[write_src_count][i*32 + j];
                            end
                            else begin
                                src_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (src_data_in_addr + write_src_count * four_byte_num * 4 + i * 4, src_data_tmp_reg, write_src_resp);
                end
                process_busy = 0;
                write_one_src_data_done <= 1;
                @(posedge clk);
                write_one_src_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        dst_write_data_finish <= 0;
        write_dst_run_flag <= 0; 
        write_dst_count = 0;
        count_operate_depth_by_bitwidth_and_depth (dst_c_bitwidth, dst_DEPTH, dst_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            dst_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_dst_run_flag <= 1; 
            write_dst_count = 0;
        end
        if (write_one_dst_data_done === 1) begin
            write_dst_count = write_dst_count + 1;
            if (write_dst_count == dst_OPERATE_DEPTH) begin
                write_dst_run_flag <= 0; 
                dst_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_dst
    integer write_dst_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] dst_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = dst_c_bitwidth;
    process_num = 2;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_2_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_dst_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write dst data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (dst_c_bitwidth < 32) begin
                        dst_data_tmp_reg = mem_dst[write_dst_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < dst_c_bitwidth) begin
                                dst_data_tmp_reg[j] = mem_dst[write_dst_count][i*32 + j];
                            end
                            else begin
                                dst_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (dst_data_in_addr + write_dst_count * four_byte_num * 4 + i * 4, dst_data_tmp_reg, write_dst_resp);
                end
                process_busy = 0;
                write_one_dst_data_done <= 1;
                @(posedge clk);
                write_one_dst_data_done <= 0;
            end   
            process_2_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_src_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [src_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (src_c_bitwidth , factor);
  fp = $fopen(`TV_IN_src ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_src); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < src_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
...
// Read data from file 
initial begin : read_dst_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [dst_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] mem_tmp; 
  reg [ 8*5 : 1] str;
  integer transaction_idx; 
  integer i; 
  transaction_idx = 0; 
  mem_tmp [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (dst_c_bitwidth , factor);
  fp = $fopen(`TV_IN_dst ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_dst); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < dst_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
