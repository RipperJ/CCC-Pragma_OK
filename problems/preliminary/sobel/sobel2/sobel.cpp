...
void sobel(PIXEL src[HEIGHT*WIDTH], PIXEL dst[(HEIGHT-2)*(WIDTH-2)], int rows, int cols)
{
	...
	long long temp = 0;
	long long tempx[HEIGHT];
	#pragma HLS ARRAY_PARTITION variable=tempx complete dim=1
	...
	for (i = 0; i < HEIGHT; ++i) {
		...
		tempx[i] = ((long long)(src[i * WIDTH]) << 32) | ((long long)(src[i * WIDTH + 1]) << 16);
	}
	...
	for (i = 0; i < HEIGHT; ++i) {
		for (j = 0; j < WIDTH - 2; ++j) {
			...
			tempx[i] |= ((long long)(src[i * WIDTH + j + 2]));
			src_temp[i][j] = tempx[i];
			tempx[i] = (tempx[i] << 16) & 0x0000FFFFFFFF0000;
		}
	}
