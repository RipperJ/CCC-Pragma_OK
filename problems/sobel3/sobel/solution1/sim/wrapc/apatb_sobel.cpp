    ...
    string AESL_num;{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<16> > gmem_pc_buffer(1839204);
          int i = 0;
          ...
          if (i > 0) {{
            ...
            ((short*)__xlx_apatb_param_src)[j] = gmem_pc_buffer[i].to_int64();
          }
            for (int j = 0, e = 917604; j < e; j += 1, ++i) {
            ((short*)__xlx_apatb_param_dst)[j] = gmem_pc_buffer[i].to_int64();
          }}}
        } // end transaction
      } // end file is good
...
sc_bv<16> __xlx_tmp_lv = ((short*)__xlx_apatb_param_src)[j];
  ...
  __xlx_offset_byte_param_dst = 921600*2;
...
sc_bv<16> __xlx_tmp_lv = ((short*)__xlx_apatb_param_dst)[j];
  ...
  tcl_file.set_num(1839204, &tcl_file.gmem_depth);
...
// print src Transactions
{
  ...
  {
    sc_bv<64> __xlx_tmp_lv = __xlx_offset_byte_param_src;
...
// print dst Transactions
{
  ...
  {
    sc_bv<64> __xlx_tmp_lv = __xlx_offset_byte_param_dst;

    sprintf(__xlx_sprintf_buffer.data(), "%s\n", __xlx_tmp_lv.to_string(SC_HEX).c_str());
    aesl_fh.write(AUTOTB_TVIN_dst, __xlx_sprintf_buffer.data()); 
  }
  tcl_file.set_num(1, &tcl_file.dst_depth);
  sprintf(__xlx_sprintf_buffer.data(), "[[/transaction]] \n");
  aesl_fh.write(AUTOTB_TVIN_dst, __xlx_sprintf_buffer.data());
}
